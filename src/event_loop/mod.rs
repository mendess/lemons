...
fn build_line(config: &Config, index: usize, monitor: u8, line: &mut String) {
    line.clear();
    ...
    Alignment::into_enum_iter()
        ...
        .for_each(|(al, blocks)| {
            ...
            blocks
                .iter()
                .filter(|b| !b.last_run[monitor].is_empty())
                .filter(|b| b.layer == current_layer())
                .map(|b| DisplayBlock(b, index, monitor))
...
#[inline(always)]
pub fn debug(line: &str, up: &str) {
    if cfg!(debug_assertions) {
        eprintln!("[{:?}] {}", chrono::Utc::now(), line);
    } else {
        eprintln!("[{:?}] bar update '{}'", chrono::Utc::now(), up);
    }
}
